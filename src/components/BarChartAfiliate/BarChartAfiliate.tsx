...
export function BarChartGraphicAfiliate({ data }: { data: IData }) {
      ...
      <CardHeader>
        ...
        <CardDescription>
          <span className="flex gap-1 items-center">
            ...
            {!data?.total_comissao_total ? '$ 0' : `$ ${data?.total_comissao_total?.toFixed(2).replace(".", ",")}`}
          </span>
        </CardDescription>
      </CardHeader>
      <CardContent>
        <ChartContainer config={chartConfig}>
          <BarChart accessibilityLayer data={data.data}>
            ...
            />
            <Bar dataKey="data.comissao_total" fill="#85FF4C" radius={4} />
          </BarChart>
        </ChartContainer>
      </CardContent>
    </Card>
